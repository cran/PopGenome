...
\\
\\
\textcolor{blue}{The following sections explain how to use the PopGenome package. Detail informations about the functions and their corresponding parameter can be found in the PopGenome manual on CRAN.}
\section{Install PopGenome}
\textcolor{blue}{Install the package via R}
\begin{Schunk}
\begin{Sinput}
...
(An example FASTA-file can be found in the \textit{data} subdirectory of the PopGenome package on CRAN. Alignment formats like \textit{Phylip, MEGA, MAF} are also accepted. \\ 
\textbf{Note:} valid nucleotides are A,a,C,c,T,t,U,u,G,g,-(gap),N,n,?(unknown). 
Internally those nucleotides are coded into numeric values:}
\begin{itemize}
...
When printing \texttt{GENOME.class} we get some informations about the main 
methods provided by PopGenome and how to get the results. The \texttt{GENOME} class is the input for every function printed below.\\
\textbf{Note:} \texttt{GENOME.class} ist just a variable, you can choose here 
whatever you want.}
\begin{Schunk}
\begin{Sinput}
> GENOME.class
...
another class and its slots are accessable with the @ operator. See also 
...
zeros (mayor allele) and ones (minor allele). rows=individuals.
...
of SNPs where exactly one individual causes a SNP.} \\
...
\textcolor{blue}{position of polyallelic sites (>2 nucleotides)} \\
...
\textbf{codons} \\
\textcolor{blue}{a list of length=n.snps. The codon changes 
are represented as numerical values.  \\
This slot is only available for data in alignment format. \\
For SNP data we provide the function \texttt{set.synnonsyn} because 
of memory issues. See also \texttt{get.codons} for some detail informations about the codon-changes and
\texttt{codontable()} to define your own genetic code.} \\ 
\textbf{<FEATURE>SNPS} \\
...
in the FASTA folder. The GFF-files should have the same names as the corresponding FASTA-files (in this example: \textit{4CL1tl.gff, C4Htl.gff and CADtl.gff}) to ensure the right matching.}
\begin{Schunk}
\begin{Sinput}
> GENOME.class <- readData("FASTA",gffpath="GFF")
\end{Sinput}
\end{Schunk}
\textcolor{blue}{Which of the first 10 SNPs (\texttt{[1:10]}) of the second (\texttt{[[2]]}) alignment are part of an synonymous mutation ?}
\begin{Schunk}
\begin{Sinput}
...
\textcolor{blue}{
PopGenome can scan the data based on the features defined in the GFF file.
In this example we are splitting into \texttt{coding} (CDS) regions. 
...
PopGenome will only consider SNPs where the outgroup is monomorh.
The monomorhic nucleotide is than automatically defined as the major allele 0 
(non mutant).}
...
(check out the PopGenome manual on CRAN).
...
\end{Schunk}
\textcolor{blue}{
If we have read in the data with the corresponding 
GFF files PopGenome can also analyse subsites like
...
\end{Soutput}
\end{Schunk}
\textcolor{blue}{The \texttt{splitting.data} function just transforms the 
class into another object of class \texttt{GENOME}, as a consequence we can apply all methods to the transformed 
class \texttt{GENOME.class.split}. Lets for example analyse all non-synonymous
...
Every time the input is an object of class \texttt{GENOME}. 
}
\subsection{The slot region.stats}
\textcolor{blue}{
The slot \texttt{region.stats} includes some site-specific statistics or values 
that can not be shown in a multi-locus-scale. See also the section \texttt{PopGenome classes}.
}
\begin{Schunk}
\begin{Sinput}
...
\textcolor{blue}{The nucleotide diversity (average pairwaise nucleotide differences) within and between the 
populations. Have to be devided by the slot \texttt{GENOME.class@n.sites}
(see also \texttt{diversity.stats})} \\
\textbf{haplotype.diversity} \\
\textcolor{blue}{The haplotype diversity (average pairwaise haplotype differences) within and between the populations.
(see also: \texttt{diversity.stats})} \\
\textbf{haplotype.counts} \\
\textcolor{blue}{A vector of length=n.indivuals. Number of times a sequence
of a specific indivual-sequence appears in the whole population} \\
...
some linkage disequilibrium measurenments for each pair of SNP 
...
\\
\textcolor{blue}{ 
PopGenome tries to concatenate the data if the parameter \texttt{whole.data} is switched to \texttt{TRUE}.
This mechanism enables the user to calculate really big data which can be splitted into smaller chunks stored in the input folder. PopGenome is able to concatenate them afterwards. Functions like \texttt{readVCF} and \texttt{readSNP} will do this automatically.
...
\\
\textcolor{blue}{\texttt{type=2}: Scanning the nucleotides }
...
used to generate the x-values for e.g, a plot along the chromosome.}
\begin{Schunk}
\begin{Sinput}
...
PopGenome can handle SNP-data formats like VCF (1000 Genome project), HapMap 
and .SNP (1001 Genome project). VCF files can be read in with the function 
\texttt{readData(,format="VCF")}. The VCF file (same as with alignments) have to
be stored in a folder. To study whole genomes, VCFs could be splitted in fairly 
sized chunks (by position) which should be numbered consecutively and stored in one folder. 
PopGenome can concatenate them afterwards in the 
PopGenome framework. Alternatively use the function \texttt{readVCF} which 
can read in a tabixed VCF-file like it is published from the 1000 Genome project.
\texttt{readVCF} supports fast access of defined subregions of the file and automatically
splits the data in chunks in cases when the region of interest is too big for the 
available RAM.\\
The function \texttt{readSNP} reads data published from the 1001 Genome project (Arabidopsis), where the \textit{quality-variant.txt} files, which include variant calls from every single individual, have to be stored in a folder.
The \texttt{readData} function can also read HapMap data. (\texttt{readData(,format="HapMap")})   
In case of using SNP-data the \texttt{FAST} parameter can be switched to TRUE.
\texttt{readData(, format="VCF", FAST=TRUE)} !!!  
Example files can be found in the subdirectory \textit{data} of the 
PopGenome package.

\subsection{Example}
\textcolor{blue}{Reading data from the 1001 Genome project (Arabidopsis)}
\begin{Schunk}
\begin{Sinput}
...
\end{Sinput}
\end{Schunk}
\textcolor{blue}{We can split the data into \texttt{genes, exons, introns, UTRs and coding} regions if the features are present in the GFF file. See also \texttt{get.gff.info} in the manual.}
...
\end{Sinput}
\end{Schunk}
\textcolor{blue}{After reading in the full range of methods can be applied to 
this data}
...
%\begin{center}
%\begin{figure}
\includegraphics[scale=0.5]{PopGenome_classes.pdf}
%\end{figure}
%\end{center}
...
polymorphism (SNP) seperately an verify if the SNP is part of a synonymous or nonsynonymous change. When there is a unknown or gap position in the entire site nucleotide-triplet of a specific individual, PopGenome will ignore those sequences and will try to find a valid codon and will interpret this change. 
If there is one nonsynonymous change PopGenome will set this SNP as 
a nonsynonymous SNP, also when there are additional synonymous changes. However, the slot \texttt{GENOME.class@region.data@codons} includes 
all codon changes and the function \texttt{get.codons} will also give more inside. If neccessary the user can redefine the syn/nonsyn changes by manipulating the \texttt{GENOME.class@region.data@synonymous} slot or define subpositions of interest with the \texttt{splitting.data} function. \\
